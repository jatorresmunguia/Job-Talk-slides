# Job Talk

**A computational approach to risk factors for intimate partner violence against women and girls in Mexico**

Job talk prepared for the **Tecnológico de Monterrey**  
**Friday, August 28, 2026**

## About

This repository contains the Quarto source code, data, and supporting materials for the job talk.

## Presentation

- **Title:** A computational approach to risk factors for intimate partner violence against women and girls in Mexico
- **Date:** August 28, 2026
- **Institution:** Tecnológico de Monterrey
- **Author:** Juan Armando Torres Munguía, PhD.

## Reproducibility

The presentation was developed using [Quarto](https://quarto.org/) and R.

To render the presentation:

```bash
quarto render